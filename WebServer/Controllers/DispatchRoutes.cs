...
namespace EggLink.DanhengServer.WebServer.Controllers
{
    ...
    public class DispatchRoutes : ControllerBase
    {
        ...
        {
            var usernameLoginHandler = new UsernameLoginHandler(_httpContextAccessor, _userActivityHandler);
            return usernameLoginHandler.Handle(req.account!, req.password!, req.is_crypto);
        }

        [HttpPost("/hkrpg_global/account/ma-passport/api/appLoginByPassword")]
        public JsonResult AppLoginByPassword([FromBody] NewLoginReqJson req)
        {
            var newUsernameLoginHandler = new NewUsernameLoginHandler();
            return newUsernameLoginHandler.Handle(req.account!, req.password!);
        }

        [HttpPost("/hkrpg_global/mdk/shield/api/verify")]
        public JsonResult Verify([FromBody] VerifyReqJson req)
        {
            var tokenLoginHandler = new TokenLoginHandler();
            return tokenLoginHandler.Handle(req.uid!, req.token!);
        }

        [HttpPost("/hkrpg_global/combo/granter/login/v2/login")]
        public JsonResult LoginV2([FromBody] LoginV2ReqJson req)
        {
            var comboTokenGranterHandler = new ComboTokenGranterHandler();
            return comboTokenGranterHandler.Handle(req.app_id, req.channel_id, req.data!, req.device!, req.sign!);
        }
        ...
        public JsonResult GetFp([FromQuery] string device_fp)
        {
            ...
            return fingerprintHandler.GetFp(device_fp);
        }
